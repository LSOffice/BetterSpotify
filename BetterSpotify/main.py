import wx, wx.adv, spotipy, os, requests
...
class Frame(wx.Frame):
    def __init__(self, title="BetterSpotify", pos=(550, 550)):
    ...
    def OnInit(self):
        ...
        self.CredentialsScene.button1.Bind(wx.EVT_BUTTON, self.onSwitchFromCredentials)
        self.MainScene.Hide()
    ...
    def onSwitchFromCredentials(self, event):
        if self.CredentialsScene.textbox1.GetValue() == "" or self.CredentialsScene.textbox1.GetValue() == "Enter your spotify username...":
        ...
        token = spotipy.util.prompt_for_user_token(self.CredentialsScene.textbox1.GetValue(), 'user-read-currently-playing', redirect_uri='http://127.0.0.1/callback')
        if token:
            sp = spotipy.Spotify(auth=token)

        open("./cache/config.txt", "a").write(self.CredentialsScene.textbox1.GetValue())
        dlg = wx.RichMessageDialog(parent=None,
...
class CredentialsScene(wx.Panel):
    def __init__(self, parent):
        ...
        holderText1 = wx.StaticText(self, wx.ID_ANY, "abcd")
...
class MainScene(wx.Panel):
    def __init__(self, parent):
        super().__init__(parent=parent)
        self.token = spotipy.util.prompt_for_user_token(open("./cache/config.txt", "r").read(),
        ...
        with open(f"{sp.currently_playing()['item']['album']['artists'][0]['name']}_{sp.currently_playing()['item']['name']}.jpg", 'wb') as handler:
        ...
        holderText1.SetForegroundColour((255, 255, 255))
        png = wx.StaticBitmap(self, -1, wx.Bitmap(f"{sp.currently_playing()['item']['album']['artists'][0]['name']}_{sp.currently_playing()['item']['name']}.jpg", wx.BITMAP_TYPE_ANY))
